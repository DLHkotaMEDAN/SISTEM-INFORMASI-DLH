...
import { Plus, Trash2, Save, ArrowLeft, FileText, Fuel } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { showSuccess, showError } from '@/utils/toast';
import { Report, ReportCategory, Location, Task, Photos, Equipment, HeavyEquipment, Personnel, FuelUsage } from '@/types/report';
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 max-w-4xl mx-auto pb-20">
        ...
        <div className="space-y-6">
          ...
          {taskFields.map((field, index) => (
            <Card key={field.id} className="border-l-4 border-l-blue-400">
              <CardContent className="p-6 space-y-4">
                ...
                <FormField control={form.control} name={`tasks.${index}.description`} render={({ field }) => (<FormItem><FormLabel>Uraian Kegiatan</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
          ...
          <Button type="button" variant="outline" className="w-full border-dashed py-6" onClick={() => appendTask({ description: "", location: { street: "", village: "", subDistrict: "" } })}><Plus className="mr-2 h-4 w-4" /> Tambah Kegiatan</Button>
        </div>
        ...
        <Card className="border-t-4 border-t-red-500">
          ...
          <CardContent className="space-y-6">
            ...
            <Button type="button" variant="outline" className="w-full border-dashed" onClick={() => appendHeavy({ type: "", quantity: 1, fuel: { pertamax: 0, dexlite: 0, solar: 0 } })}><Plus className="h-4 w-4 mr-2" /> Tambah Alat Berat</Button>
          </CardContent>
        </Card>
        ...
        <Card className="border-t-4 border-t-cyan-500">
          <CardHeader><CardTitle className="text-lg">Personil & Volume</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <FormField control={form.control} name="volume" render={({ field }) => (<FormItem><FormLabel>Volume ({getUnitByCategory(selectedCategory)})</FormLabel><FormControl><Input type="number" {...field} /></FormControl></FormItem>)} />
          </CardContent>
        </Card>
