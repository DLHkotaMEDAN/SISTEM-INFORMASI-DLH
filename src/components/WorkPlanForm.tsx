...
const formSchema = z.object({
  ...
  const isGlobalStyle = ["Tim Pohon", "Tim Babat", "Tim Siram"].includes(data.category);
...
const WorkPlanForm = ({ initialData, isEditing = false }: { initialData?: WorkPlan; isEditing?: boolean }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: initialData ? {
      ...
      globalTools: ["Tim Pohon", "Tim Babat", "Tim Siram"].includes(initialData.category) ? initialData.items[0].tools : [{ name: "", unit: 1, usage: "" }],
      globalCoordinator: ["Tim Pohon", "Tim Babat", "Tim Siram"].includes(initialData.category) ? initialData.items[0].coordinator : "",
      globalMembers: ["Tim Pohon", "Tim Babat", "Tim Siram"].includes(initialData.category) ? initialData.items[0].personnel.members : 0,
    } : {
  ...
  const isGlobalStyle = ["Tim Pohon", "Tim Babat", "Tim Siram"].includes(selectedCategory);
  ...
  const handleAddClick = () => {
    ...
    appendItem({
      description: isGlobalStyle ? lastItem?.description || "" : "",
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const processedItems = values.items.map(item => {
        if (isGlobalStyle) {
          return { ...item, tools: cleanGlobalTools, coordinator: values.globalCoordinator || "", personnel: { members: values.globalMembers || 0 } };
        } else {
          return { ...item, tools: item.tools.filter(t => t.name && t.name.trim() !== "") };
        }
      });
