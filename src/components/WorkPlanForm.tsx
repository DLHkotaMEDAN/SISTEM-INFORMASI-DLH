...
const WorkPlanForm = ({ initialData, isEditing = false }: { initialData?: WorkPlan; isEditing?: boolean }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: initialData ? {
      ...
      globalTools: (initialData.category === "Tim Pohon" || initialData.category === "Tim Siram") ? initialData.items[0].tools : [{ name: "", unit: 1, usage: "" }],
      globalCoordinator: (initialData.category === "Tim Pohon" || initialData.category === "Tim Siram") ? initialData.items[0].coordinator : "",
      globalMembers: (initialData.category === "Tim Pohon" || initialData.category === "Tim Siram") ? initialData.items[0].personnel.members : 0,
    } : {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 max-w-5xl mx-auto pb-20">
        ...
        <div className="space-y-6">
          ...
          {itemFields.map((item, index) => (
            <Card key={item.id} className="border-l-4 border-l-blue-400 shadow-md">
              <CardContent className="p-6 space-y-6">
                <div className="flex justify-between items-center border-b pb-4"><Badge className="bg-blue-600">Lokasi #{index + 1}</Badge>{itemFields.length > 1 && (<Button type="button" variant="ghost" size="sm" className="text-red-500" onClick={() => removeItem(index)}><Trash2 className="h-4 w-4 mr-1" /> Hapus Lokasi</Button>)}</div>
                ...
                {!isGlobalStyle && (
                  <>
                    <div className="pt-4 border-t space-y-4">
                      <div className="flex items-center justify-between"><FormLabel className="flex items-center gap-2 text-blue-700 font-bold"><Wrench size={16} /> Alat Operasional</FormLabel><Button type="button" variant="outline" size="sm" className="h-8" onClick={() => { const current = form.getValues(`items.${index}.tools`); form.setValue(`items.${index}.tools`, [...current, { name: "", unit: 1, usage: "" }]); }}><Plus size={14} className="mr-1" /> Tambah Alat</Button></div>
          ...
          <Button type="button" variant="outline" className="w-full border-dashed py-8 text-blue-600 font-bold" onClick={() => appendItem({
            ...
            remarks: ""
          })}><Plus className="mr-2 h-5 w-5" /> Tambah Lokasi Kerja Baru</Button>
        </div>
        <div className="flex justify-end gap-4"><Button type="button" variant="outline" onClick={() => navigate(-1)}>Batal</Button><Button type="submit" disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-700 px-8">{isSubmitting ? "Menyimpan..." : "Simpan Rencana Kerja"}</Button></div>
      </form>
    </Form>
  );
};
