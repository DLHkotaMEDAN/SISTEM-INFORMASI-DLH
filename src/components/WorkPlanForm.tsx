...
import { Plus, Trash2, Save, ArrowLeft, Loader2, MapPin, Users, Wrench, FileText, MessageSquare, ClipboardList, ShieldAlert, Check, HelpCircle, Copy, AlertCircle, Ban } from 'lucide-react';
...
const WorkPlanForm = ({ initialData, isEditing = false }: { initialData?: WorkPlan; isEditing?: boolean }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: initialData ? {
      ...
      isNihil: initialData.items[0]?.description.includes("NIHIL"),
      ...
      remarks: initialData.items[0]?.remarks || "",
    } : {
  ...
  useEffect(() => {
    if (isNihil) {
      const nihilItem = {
        ...
        tools: [],
        coordinator: categoryCoordinatorMapping[selectedCategory] || "-",
        personnel: { members: 0 },
        basis: "-",
        remarks: "-",
        uiMode: 'full' as const
      };
      replaceItems([nihilItem]);
    } else if (!isEditing && !isDuplicateMode) {
      const defaultDesc = defaultActivityMapping[selectedCategory] || "";
      replaceItems([{ description: defaultDesc, location: { street: "", village: [""], subDistrict: "" }, tools: isGlobalStyle ? [] : [{ name: "", unit: 1, usage: "" }], coordinator: categoryCoordinatorMapping[selectedCategory] || "", personnel: { members: 0 }, basis: "", remarks: "", uiMode: 'full' }]);
    }
  }, [isNihil, selectedCategory, replaceItems, isEditing, isDuplicateMode, isGlobalStyle]);
  ...
  }, [selectedCategory, isGlobalStyle, isEditing, isDuplicateMode, form, isNihil]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const processedItems = values.items.map(item => {
        ...
        if (values.isNihil) return { ...rest, remarks: values.remarks };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 max-w-5xl mx-auto pb-20">
        ...
        <Card className="border-t-4 border-t-blue-500">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full border">
              ...
              <span className="text-[10px] font-bold uppercase text-slate-600">Rencana Nihil</span>
        ...
        {isNihil ? (
          <Card className="border-t-4 border-t-red-500 bg-red-50/30">
            <CardHeader><CardTitle className="text-lg flex items-center gap-2 text-red-700"><Ban className="h-5 w-5" /> Alasan Tidak Ada Rencana</CardTitle></CardHeader>
            <CardContent>
              <FormField control={form.control} name="remarks" render={({ field }) => (
                <FormItem>
                  <FormLabel className="font-bold text-red-600">Keterangan / Alasan (Wajib)</FormLabel>
                  <FormControl><Input {...field} placeholder="Contoh: Libur Nasional / Seluruh personil dialihkan ke kegiatan lain..." className="h-12 border-red-200 focus:ring-red-500" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </CardContent>
              ...
              <Card className="border-t-4 border-t-orange-500 bg-orange-50/30">
                ...
                <CardContent className="space-y-6 pt-4">
                  ...
                  <div className="space-y-4 pt-4 border-t border-orange-200">
                    <div className="flex items-center justify-between"><FormLabel className="flex items-center gap-2 text-orange-700 font-bold"><Wrench size={16} /> Alat Operasional Tim</FormLabel><Button type="button" variant="outline" size="sm" className="h-8 border-orange-200 text-orange-700 hover:bg-orange-100" onClick={() => { const current = form.getValues("globalTools") || []; form.setValue("globalTools", [...current, { name: "", unit: 1, usage: "" }]); }}><Plus size={14} className="mr-1" /> Tambah Alat</Button></div>
