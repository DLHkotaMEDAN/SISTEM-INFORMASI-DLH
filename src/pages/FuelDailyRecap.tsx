    ...
    <div className="min-h-screen bg-slate-50 p-0 md:p-8">
      ...
      <div className="print-area bg-white p-10 mx-auto shadow-lg border min-h-[210mm] w-full max-w-[297mm]">
        ...
        <div className="text-center mb-8">
          <h3 className="text-lg font-bold underline uppercase">REKAP HARIAN PEMAKAIAN BBM & OLI</h3>
          <p className="font-bold">Tanggal: {format(new Date(selectedDate), 'dd MMMM yyyy', { locale: localeId })}</p>
        </div>
        ...
        <table className="w-full border-collapse border-2 border-black text-[10px] table-fixed">
          <thead>
            <tr className="bg-slate-100">
              ...
              <th className="border-2 border-black p-1 w-[130px]">Kendaraan / Alat Operasional</th>
              <th className="border-2 border-black p-1 w-[90px]">Jenis BBM / OLI</th>
              ...
              <th className="border-2 border-black p-1 w-[90px]">Wilayah</th>
            ...
            {reports.length > 0 ? (
              reports.flatMap((report, rIdx) => 
                report.items.map((item, iIdx) => (
                  <tr key={`${rIdx}-${iIdx}`}>
                    <td className="border-2 border-black p-1 text-center">{iIdx === 0 && report.items.length > 1 ? `${rIdx + 1}.${iIdx + 1}` : rIdx + 1}</td>
                    <td className="border-2 border-black p-1 text-center">{report.date}</td>
                    ...
                    <td className="border-2 border-black p-1 italic text-[9px]">
              ...
              <tr><td colSpan={8} className="border-2 border-black p-8 text-center italic text-slate-400">Tidak ada data untuk tanggal ini</td></tr>
            )}
          </tbody>
        </table>
