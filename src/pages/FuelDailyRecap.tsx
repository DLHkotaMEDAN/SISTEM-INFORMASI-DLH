...
const FuelDailyRecap = () => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      if (visibleColumns.vehicle) columns.push({ header: 'Kendaraan / Alat Operasional', key: 'vehicle', width: 24 });
          ...
          <table className="w-full min-w-[1000px] border-collapse border-2 border-black text-[10px] table-fixed">
            ...
            <tbody>
              {Object.keys(groupedByRegion).length > 0 ? (
                ...
                <tr><td colSpan={9} className="border-2 border-black p-8 text-center italic text-slate-400">Tidak ada data untuk kriteria ini</td></tr>
              )}
            </tbody>
          </table>
        </div>
