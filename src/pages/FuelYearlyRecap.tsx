    ...
    <div className="min-h-screen bg-slate-50 p-0 md:p-8">
      ...
      <div className="print-area bg-white p-10 mx-auto shadow-lg border min-h-[210mm] w-full max-w-[297mm]">
        ...
        <div className="text-center mb-8">
          <h3 className="text-lg font-bold underline uppercase">REKAP TAHUNAN PEMAKAIAN BBM & OLI</h3>
          <p className="font-bold">Tahun: {selectedYear}</p>
        </div>

        <table className="w-full border-collapse border-2 border-black text-[7px] table-fixed">
          <thead>
            <tr className="bg-slate-100">
              <th className="border-2 border-black p-1 w-[25px]">No</th>
              <th className="border-2 border-black p-1 w-[50px]">Tanggal</th>
              <th className="border-2 border-black p-1 w-[110px]">Kendaraan / Alat Operasional</th>
              <th className="border-2 border-black p-1 w-[80px]">Jenis BBM / OLI</th>
              <th className="border-2 border-black p-1 w-[170px]">Lokasi Kerja</th>
              <th className="border-2 border-black p-1 w-[90px]">Tim / Operator Utama</th>
              <th className="border-2 border-black p-1 w-[80px]">Wilayah</th>
              <th className="border-2 border-black p-1 w-[115px]">Keterangan</th>
            </tr>
          </thead>
          <tbody>
            {reports.length > 0 ? (
              reports.flatMap((report, rIdx) => 
                report.items.map((item, iIdx) => (
                  <tr key={`${rIdx}-${iIdx}`}>
                    <td className="border-2 border-black p-1 text-center">{rIdx + 1}</td>
                    <td className="border-2 border-black p-1 text-center">{format(parseISO(report.date), 'dd/MM/yy')}</td>
                    <td className="border-2 border-black p-1">{item.vehicle_operator}</td>
                    <td className="border-2 border-black p-1 text-center">
                      {item.fuel_type} <br/> 
                      <span className="font-bold">{item.fuel_type === 'Oli' ? `${item.amount} L` : `Rp ${item.amount.toLocaleString('id-ID')}`}</span>
                    </td>
                    <td className="border-2 border-black p-1 break-words">
                      {item.location.street}
                      {item.location.village && `, Kel. ${item.location.village}`}
                      {item.location.subDistrict && `, Kec. ${item.location.subDistrict}`}
                    </td>
                    <td className="border-2 border-black p-1 text-center font-bold">{report.team}</td>
                    <td className="border-2 border-black p-1 text-center">{report.region}</td>
                    <td className="border-2 border-black p-1 italic text-[6px]">
                      {[item.item_remarks, iIdx === 0 ? report.remarks : ""].filter(Boolean).join(" | ") || "-"}
                    </td>
                  </tr>
                ))
              )
            ) : (
              <tr><td colSpan={8} className="border-2 border-black p-8 text-center italic text-slate-400">Tidak ada data untuk tahun ini</td></tr>
            )}
          </tbody>
        </table>
        ...
        <div className="mt-12 flex justify-end">
          <div className="text-center w-64">
            ...
            <p className="font-bold mt-1">Admin Wilayah 4</p>
